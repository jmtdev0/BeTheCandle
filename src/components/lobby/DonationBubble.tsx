...
import { SatelliteUser } from "@/components/lobby/InteractiveSphere3D";

type OrbitBodyType = "moon" | "comet" | "drone" | "capsule" | "ring";
...
const OrbitVisualization = ({
  ...
  const uniqueOrbitDistances = useMemo(() => {
    const set = new Set<number>();
    satellites.forEach((sat) => set.add(sat.orbitDistance));
    return Array.from(set.values()).sort((a, b) => a - b);
  }, [satellites]);

  return (
    <div className="absolute inset-0">
      {uniqueOrbitDistances.map((distance, index) => {
        const normalized = Math.min(0.9, Math.max(0.35, distance / 6 + 0.35));
        const ringRadius = radiusPx * normalized;
        return (
          <div
            key={`orbit-ring-${distance}-${index}`}
      ...
      {satellites.map((satellite, index) => {
        ...
        const normalizedDistance = Math.min(0.9, Math.max(0.35, satellite.orbitDistance / 6 + 0.35));
        const distance = radiusPx * normalizedDistance;
        const x = Math.cos(angle) * distance;
        const y = Math.sin(angle) * distance;
        const sizePx = Math.max(28, Math.min(radiusPx * 0.35, radiusPx * satellite.size * 0.6));
        ...
        return (
          <motion.button
            key={satellite.id}
            type="button"
            className={`absolute flex items-center justify-center rounded-full border transition
              ${isActive ? "border-amber-300 shadow-[0_0_30px_rgba(251,191,36,0.3)]" : "border-transparent shadow-[0_0_18px_rgba(59,130,246,0.2)]"}`}
            style={{
              width: `${sizePx}px`,
              height: `${sizePx}px`,
              left: `calc(50% + ${x}px)`,
              top: `calc(50% + ${y}px)`,
              transform: "translate(-50%, -50%)",
              background: `radial-gradient(circle at 30% 30%, ${satellite.color}cc, ${satellite.color}40)`,
              color: "#fff",
              fontSize: Math.max(12, sizePx * 0.35),
              cursor: "pointer",
            }}
            whileHover={{ scale: 1.08 }}
            whileTap={{ scale: 0.94 }}
            onClick={() => onSatelliteSelect?.(satellite)}
            aria-label={`Seleccionar satélite ${satellite.displayName}`}
          >
            <span className="drop-shadow-[0_0_6px_rgba(0,0,0,0.35)]">{satellite.avatar ?? "🛰️"}</span>
          </motion.button>
        );
      })}
    </div>
...
const ORBIT_TYPES: OrbitBodyType[] = ["moon", "comet", "drone", "capsule", "ring"];
              ...
              >
                <div className="flex items-start justify-between text-slate-100">
                  <div>
                    ...
                    <h2 className="mt-1 text-2xl font-semibold">{selectedSatellite.displayName}</h2>
                  </div>
                  <div className="text-3xl drop-shadow-lg">{selectedSatellite.avatar ?? "🛰️"}</div>
                </div>
                {selectedSatellite.bio && (
                  <p className="mt-4 text-sm leading-relaxed text-slate-300">{selectedSatellite.bio}</p>
                )}
        ...
        {miniBubbles.map((bubble) => (
          <motion.div
            ...
            animate={{
              scale: [0, 1, 0.8],
              opacity: [0, 1, 0],
              x: typeof window !== "undefined" ? window.innerWidth / 2 - bubble.x : 0,
              y: typeof window !== "undefined" ? window.innerHeight / 2 - bubble.y : 0,
            }}
